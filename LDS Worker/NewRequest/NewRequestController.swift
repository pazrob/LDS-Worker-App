...
class NewRequestController: UIViewController, UINavigationBarDelegate {
    ...
    private func setUpUserInfo(){
        ...
        guard let userPhoto = ImageService.userPhoto else { return }
    ...
    private func setUpViews(){
        ...
        view.addConstraint(NSLayoutConstraint(item: noteField, attribute: .top, relatedBy: .equal, toItem: noteLabel, attribute: .bottom, multiplier: 1, constant: 5))
        view.addConstrainsWithFormat(format: "H:|-14-[v0]-14-|", views: noteField)
        noteField.heightAnchor.constraint(equalToConstant: 50)
        
        view.addConstraint(NSLayoutConstraint(item: divider3, attribute: .top, relatedBy: .equal, toItem: noteField, attribute: .bottom, multiplier: 1, constant: 0))
        view.addConstrainsWithFormat(format: "H:|-14-[v0]|", views: divider3)
        view.addConstrainsWithFormat(format: "V:[v0(0.3)]", views: divider3)
        
        view.addConstraint(NSLayoutConstraint(item: fromLabel, attribute: .top, relatedBy: .equal, toItem: noteField, attribute: .bottom, multiplier: 1, constant: 15))
    ...
    @IBAction func handleSend(_ sender: UIBarButtonItem) {
        ...
        sendActionSheet.addAction(UIAlertAction(title: "Send", style: .default, handler: { (action: UIAlertAction) in
            guard let noteValue = self.noteField.text else {return}
            guard let fromValue = self.dateField.text else {return}
            guard let toValue = self.timeField.text else {return}
            
            FirebaseService.performSendRequest(note: noteValue, from: fromValue, to: toValue)
            self.presentingViewController?.dismiss(animated: true, completion: nil)
        }))
