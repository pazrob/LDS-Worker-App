...
// FIX: ActivityView is not working,
...
//        textField.clearButtonMode = .whileEditing
    ...
    let passConfirmationField: UITextField = {
        let textField = UITextField()
        textField.placeholder = "Confirm Password"
        textField.clearButtonMode = .whileEditing
        var leftIcon = UIImageView(frame: CGRect(x: 0, y: 0, width: 50, height: 30))
        leftIcon.image = UIImage(named: "iconDoubleLock")?.withRenderingMode(.alwaysTemplate)
        leftIcon.tintColor = .fieldIcon
        leftIcon.contentMode = .scaleAspectFit
        textField.leftView = leftIcon
        textField.leftViewMode = UITextFieldViewMode.always
...
//        guard let passwordCheck = passConfirmationField.text else{ return }
        activityView.startAnimating()
        
        
        FirebaseService.performRegister(email: email, password: password) { (success) in
            if success {
                FirebaseService.saveUserProfile(firstNameValue: firstName, lastNameValue: lastName, emailValue: email, completion: { success in
                    if success {
                        print("User saved")
                        
                        FirebaseService.registeringAccount = true
    ...
    override func setUpViews(){
        ...
        fieldContainer.addSubview(fieldDivider3)
        fieldContainer.addSubview(passConfirmationField)
        
        addSubview(registerBtn)
        addSubview(activityView)
        
        //Constraints
        addConstrainsWithFormat(format: "V:[v0(216)]", views: fieldContainer)
        ...
        addConstrainsWithFormat(format: "V:[v0(2)]", views: fieldDivider3)
        addConstrainsWithFormat(format: "H:|[v0]|", views: fieldDivider3)
        addConstraint(NSLayoutConstraint(item: fieldDivider3, attribute: .top, relatedBy: .equal, toItem: passwordField, attribute: .bottom, multiplier: 1, constant: 0))
        
        addConstrainsWithFormat(format: "V:[v0(54)]", views: passConfirmationField)
        addConstrainsWithFormat(format: "H:|[v0]|", views: passConfirmationField)
        addConstraint(NSLayoutConstraint(item: passConfirmationField, attribute: .top, relatedBy: .equal, toItem: passwordField, attribute: .bottom, multiplier: 1, constant: 0))
        
        addConstrainsWithFormat(format: "V:[v0(42)]", views: registerBtn)
        addConstrainsWithFormat(format: "H:[v0(200)]", views: registerBtn)
        addConstraint(NSLayoutConstraint(item: registerBtn, attribute: .top, relatedBy: .equal, toItem: fieldContainer, attribute: .bottom, multiplier: 1, constant: 40))
        addConstraint(NSLayoutConstraint(item: registerBtn, attribute: .centerX, relatedBy: .equal, toItem: fieldContainer, attribute: .centerX, multiplier: 1, constant: 0))
    }
